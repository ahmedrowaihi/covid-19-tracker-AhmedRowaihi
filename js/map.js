  ...
  function STARTALL() {
    ...
    if (load === numToposToLoad) {
      ...
      document
        ...
        .addEventListener("click", function () {
          ...
          if (icon.classList.value == "fas fa-sun") {
            ...
            map.setOptions({
              styles: styless["light"],
            });
            document.cookie = "theme=light";
            document.getElementById("changeMode").style.backgroundColor =
              "#546bab";
          } else {
            ...
            document.getElementById("changeMode").style.backgroundColor =
              "#fcb103";
            document.getElementById("toDead").style.backgroundColor =
              "aqua !important";
          }
      ...
      function getCountryByCode(code) {
        return country_data.filter(function (data) {
      ...
      function getMapColoring(num, mode) {
        ...
        if (mode == "confirmed") {
          idx = Math.floor(
            (conf_colors.length - 1) * Math.min(Math.log10(num) / 5, 1)
          );
          ...
          idx = Math.floor(
            (dead_colors.length - 1) * Math.min(Math.log10(num) / 5, 1)
          );
          op = Math.floor(255 * Math.min(Math.log10(num) / 5, 1)).toString(16);
          if (op.length == 1) {
            op = "0" + op;
          ...
          idx = Math.floor(
            (rec_colors.length - 1) * Math.min(Math.log10(num) / 5, 1)
          );
          op = Math.floor(255 * Math.min(Math.log10(num) / 5, 1)).toString(16);
          if (op.length == 1) {
            op = "0" + op;
      ...
      function mapInfoWindow(
        ...
        if (document.getElementById("iwTotal").style.display == "") {
          document.getElementById("iwTotal").classList.remove("active");
        }

        document.getElementById(
          "infoCountryLegend"
        ).innerHTML = `${name}&nbsp;&nbsp; <img src="${url}"  style="max-width:30px; height:auto;" onerror="this.style.display='none'" onwidth="20"/>`;
        document.getElementById("infoUpdatedLegend").innerText = timeSince(
          new Date(updated)
        );

        // confirmed
        if (c_velocity > 1) {
          document.getElementById("infoConfirmedLegend").innerHTML =
            formatNumber(confirmed) +
            "<i class='fas fa-sort-up fa-xs' style='border: 0; color: rgb(214, 102, 121)'></i>";
        } else {
          document.getElementById(
            "infoConfirmedLegend"
          ).innerHTML = formatNumber(confirmed);
        }

        // dead
        if (dead !== null) {
          document.getElementById("infoDeadLegend").innerHTML = formatNumber(
            dead
          );
        } else {
        ...
        if (recovered !== null) {
          document.getElementById(
            "infoRecoveredLegend"
          ).innerHTML = formatNumber(recovered);
        } else {
        ...
        document.getElementById("velAct").innerHTML =
          ...
          Math.max(active_vel, 0).toLocaleString();
        document.getElementById("velDead").innerHTML =
          '<i data-feather="trending-up"></i>&nbsp; ' +
          Math.max(dead_vel, 0).toLocaleString();
        document.getElementById("velRec").innerHTML =
          '<i data-feather="trending-up"></i>&nbsp; ' +
          Math.max(rec_vel, 0).toLocaleString();
        feather.replace();
        ...
        document.getElementById("infoActiveLegend").innerHTML = formatNumber(
          confirmed - dead - recovered
        );

        // line percentages
        if (confirmed > 0) {
          document.getElementById("activePercentLegend").innerHTML =
            95 * ((confirmed - dead - recovered) / confirmed) + "%";
          document.getElementById("deadPercentLegend").innerHTML =
            95 * (dead / confirmed) + "%";
          document.getElementById("recPercentLegend").innerHTML =
            95 * (recovered / confirmed) + "%";

          document.getElementById("actSVGLegend").style.strokeDasharray =
            100 * ((confirmed - dead - recovered) / confirmed) + ", 100";
          document.getElementById("dedSVGLegend").style.strokeDasharray =
            100 * (dead / confirmed) + ", 100";
          document.getElementById("recSVGLegend").style.strokeDasharray =
            100 * (recovered / confirmed) + ", 100";
        } else {
        ...
        if (population != null && population != 0) {
          document.getElementById("infoPopLegend").innerHTML =
            (100 * (confirmed / population)).toFixed(3) + "%";
          ...
          if (region_numbers[name] != null) {
            generateGraphGivenNumbers(region_numbers[name], "timeSeriesSVG", {
        ...
        document.getElementById("iwTotal").style.display = "none";
      ...
      function createCircle(args) {
        ...
        var circleOption = {
          ...
          map: map,
          radius: Math.min(
            Math.max(Math.log(args["data"]) * mult, 3.5 * mult),
            100 * mult
          ),
          zIndex: 10,
        };
        ...
        google.maps.event.addListener(circle, "click", function (ev) {
          mapInfoWindow(
            ...
            args["url"],
            args["confirmed"],
            args["velocity_confirmed"],
            args["dead"],
            args["velocity_dead"],
            args["recovered"],
            args["velocity_recovered"],
            null,
            args["updated"]
          );
        });
      ...
      async function redrawCircles() {
        ...
        if (mode === "confirmed") {
          document.getElementById("toConfirmed").classList =
            "menu-item glow-circle activeMap";
          ...
          city_data.forEach(function (city) {
            if (city.confirmed > 0) {
              ...
              params = {
                ...
                lng: city.long,
                name: city.city_name,
                url: `https://www.countryflags.io/${city.country_code}/iso_a2`,
          ...
          document.getElementById("toDead").classList =
            "menu-item glow-circle activeMap";
          document.getElementById("toRecovered").classList = "menu-item";
          ...
          document.getElementById("toConfirmed").classList = "menu-item";
          document.getElementById("toDead").classList = "menu-item";
          document.getElementById("toRecovered").classList =
            "menu-item glow-circle activeMap";
      ...
      range.on("input", function () {
        ...
        if (state >= 75) {
          document.getElementById("thumb").classList =
            "range-slider__thumb range-slider__city_thumb";
          document.getElementById("thumbText").textContent = "Cities";
        } else if (state >= 30) {
          document.getElementById("thumb").classList =
            "range-slider__thumb range-slider__prov_thumb";
          document.getElementById("thumbText").textContent = "Provinces";
        } else {
          document.getElementById("thumb").classList =
            "range-slider__thumb range-slider__country_thumb";
      ...
      (function initAndSetupTheSliders() {
        [].forEach.call(
          document.getElementsByClassName("refineSliderContainer"),
          function (el) {
      ...
      document
        ...
        .addEventListener("click", function () {
          ...
          document.getElementById("Info").style.display = "none";
          document.getElementById("legend").style.display = "none";
          document.getElementById("iwTotal").style.display = "";
        });
      document.getElementById("dismiss").addEventListener("click", function () {
        document.getElementById("sidebar").classList.toggle("active");
      });
      document
        .getElementById("openInfo")
        .addEventListener("click", function () {
          if (document.getElementById("sidebar").classList != "active") {
            document.getElementById("sidebar").classList.add("active");
          } else {
  ...
  if (settings.graphType == "bar") {
    for (i = 0; i < len; i++) {
      ...
      thisRect.setAttribute(
        "onmousemove",
        "updateTooltip('" + numArray[i] + "', event)"
      );
      //thisRect.setAttribute("onfocus", "updateTooltip('" + numArray[i] + "', event)");
      thisRect.setAttribute(
        "onmouseleave",
        'document.getElementById("svgTooltip").style.visibility = "hidden";'
      );
    }
    //Set up axes
    ...
    for (i = 0; i < circlesSVG.length; i++) {
      ...
      //Set up tooltips
      thisCircle.setAttribute(
        "onmousemove",
        "updateTooltip('" + circlesSVG[i][2] + "', event)"
      );
